...
// GET - Liste des pharmacies (publique, pour l'inscription)
if ($method === 'GET' && $action === 'pharmacies') {
    $stmt = $pdo->prepare("
        SELECT id_pharmacie, nom, adresse 
        FROM Pharmacie 
        WHERE est_active = 1
        ORDER BY nom ASC
    ");
...
// INSCRIPTION
elseif ($method === 'POST' && $action === 'inscription') {
...
elseif ($method === 'POST' && $action === 'connexion') {
    ...
    if ($user['statut_compte'] === 'EN_ATTENTE_VALIDATION') {
        http_response_code(403);
        echo json_encode(['erreur' => 'Compte en attente de validation par un administrateur']);
        exit();
    }
    ...
    if ($user['role'] === 'PHARMACIEN') {
        $stmt = $pdo->prepare("
            SELECT p.nom AS nom_pharmacie, p.statut
            FROM Pharmacie p
            WHERE p.id_pharmacien = ?
        ");
        $stmt->execute([$user['id_utilisateur']]);
        $pharmacie = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($pharmacie) {
            $donneesSupp['nom_pharmacie']    = $pharmacie['nom_pharmacie'];
            $donneesSupp['statut_pharmacie'] = $pharmacie['statut'];
        }
    }

    if ($user['role'] === 'LIVREUR') {
        $stmt = $pdo->prepare("
            SELECT p.nom AS nom_pharmacie
            FROM Pharmacie p
            JOIN Livreur l ON l.id_pharmacie = p.id_pharmacie
            WHERE l.id_livreur = ?
        ");
        $stmt->execute([$user['id_utilisateur']]);
        $pharmacie = $stmt->fetch(PDO::FETCH_ASSOC);
